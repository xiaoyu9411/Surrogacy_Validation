...
for (l in 1:nsim){
  ...
  trialref <- unlist(mapply(rep, 1:N, each = 250))
  ...
  for (j in 1:nrow(data)){
    if(data$trt[j]==0){
      data$S[j] <-deltaS[j] * (Y[j, 1] * sqrt(2 * lambdaS[j])) ^ (1 / gammasc)
      data$T[j] <-deltaT[j] * (Y[j, 1] * sqrt(2 * lambdaT[j])) ^ (1 / gammatc)
    }
    if(data$trt[j]==1){
      data$S[j] <-deltaS[j] * (Y[j, 1] * sqrt(2 * lambdaS[j])) ^ (1 / gammase)
      data$T[j] <-deltaT[j] * (Y[j, 1] * sqrt(2 * lambdaT[j])) ^ (1 / gammate)
    }
  }
...
if(min(FU)>=5*365){
  ...
  write.table(followup[ind,],file="resultf1.txt",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
 
allSurroRes <- surrosurv(data, c('clayton'), verbose = TRUE) 
 
  R2c[ind,1]<-allSurroRes$Clayton$adj$R2
  write.table(R2c[ind,],file="resultc1.txt",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
 ...
 trial<-unique(data$trialref)
  ...
  R2all[ind,]<-R2t
  write.table(R2all[ind,],file="resultall1.txt",append=TRUE,sep=",",col.names = FALSE,row.names = FALSE)
  }
}
}
